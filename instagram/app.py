...
class InstagramScraper(object):
    ...
    def scrape_hashtag(self,use_end_cursor=True):
        self.__scrape_query(self.query_hashtag_gen,use_end_cursor)
    ...
    def __scrape_query(self, media_generator, executor=concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS),use_end_cursor=True):
        ...
        try:
            for value in self.usernames:
                ...
                iter = 0
                for item in tqdm.tqdm(media_generator(value,use_end_cursor), desc='Searching {0} for posts'.format(value), unit=" media",
                                      disable=self.quiet):
    ...
    def query_hashtag_gen(self, hashtag,use_end_cursor=True):
        return self.__query_gen(QUERY_HASHTAG, QUERY_HASHTAG_VARS, 'hashtag', hashtag,use_end_cursor)
    ...
    def __query_gen(self, url, variables, entity_name, query, end_cursor=''):
        ...
        dst = self.get_dst_dir(query)
        if end_cursor==True:
            end_cursor = self.load_end_cursor(dst)
        else:
            end_cursor = ''
        #import pdb; pdb.set_trace()
        nodes, end_cursor = self.query(url, variables, entity_name, query, end_cursor)
        self.save_end_cursor(end_cursor,dst)
    ...
    def query(self, url, variables, entity_name, query, end_cursor):
    ...
    def save_end_cursor(self,end_cursor, dst='./'):
        if not os.path.exists(os.path.dirname(dst)):
            os.makedirs(os.path.dirname(dst))
        hashtag = self.usernames[0]
        dst = f'{dst}{hashtag}_end_cursor.pickle'
            
        if end_cursor:
            with open(dst, 'wb') as f:
                pickle.dump(end_cursor,f)

    def load_end_cursor(self,dst='./'):
        hashtag = self.usernames[0]
        dst = f'{dst}{hashtag}_end_cursor.pickle'
        if not os.path.isfile(dst):
           #raise FileNotFoundError("End cursor doesn't exist") 
           return ''
       
        with open(dst, 'rb') as f:
            end_cursor=pickle.load(f)
        return end_cursor
